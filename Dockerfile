...
RUN dpkg -i /tmp/semaphore_2.10.22_linux_amd64.deb 

# Copy playbooks
COPY playbooks /semaphore/playbooks

# Copy argocd manifests
COPY argocd /semaphore/argocd

# Copy semaphore config
COPY configurations.json /semaphore/config_template.json
COPY entrypoint.sh /semaphore/entrypoint.sh
RUN chmod +x /semaphore/entrypoint.sh

# Clean up
RUN rm -rf /tmp/kubespray && rm -rf /tmp/semaphore_2.10.22_linux_amd64.deb 
# Startup process
